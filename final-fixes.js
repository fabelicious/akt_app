(function(){
  ...
  async function loadTop10Scores(){
    try{
      const j=await getJson('./top10.json?ts='+Date.now());
      top10Scores=new Map();
      (j.items||[]).forEach(x=>{
        const score=Number(x.score);if(!Number.isFinite(score))return;
        if(x.wkn)top10Scores.set(String(x.wkn).trim().toUpperCase(),score);
        if(x.symbol)top10Scores.set(String(x.symbol).trim().toUpperCase(),score);
      });
      syncAllScores();
  ...
  function getGroupScore(group){
    ...
    const m=group.textContent.match(/WKN\s+([A-Z0-9]+)/i);
    const wkn=m?.[1]?.toUpperCase()||'';
    if(wkn && top10Scores.has(wkn))return top10Scores.get(wkn);
    if(symbol && top10Scores.has(symbol))return top10Scores.get(symbol);
    return null;
  }

  function applyTop10Score(group){
    const score=getGroupScore(group);if(!Number.isFinite(score))return;
    group.dataset.top10Score=String(score);
  ...
  function syncAllScores(){document.querySelectorAll('.stock-group').forEach(applyTop10Score)}
  ...
  async function copyWkn(wkn,el){
    let ok=false;
    try{if(navigator.clipboard?.writeText){await navigator.clipboard.writeText(wkn);ok=true}}catch(_){ }
    if(!ok){try{const t=document.createElement('textarea');t.value=wkn;t.setAttribute('readonly','');t.style.position='fixed';t.style.left='-9999px';document.body.appendChild(t);t.focus();t.select();ok=document.execCommand('copy');t.remove()}catch(_){ }}
    const old=el.textContent;el.textContent='WKN '+wkn+' ✓';setTimeout(()=>el.textContent=old,900)
  }

  function makeWknCopyable(root){
    root.querySelectorAll('.top10-wkn').forEach(el=>{if(el.dataset.copyReady)return;const m=el.textContent.match(/WKN\s+([A-Z0-9]+)/i);if(!m)return;el.dataset.copyReady='1';el.title='Klicken zum Kopieren';el.style.cursor='pointer';el.addEventListener('click',e=>{e.preventDefault();e.stopPropagation();copyWkn(m[1].toUpperCase(),el)})});
