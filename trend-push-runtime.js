...
function install(){const p=document.getElementById('watchlistPanel');if(!p)return;if(!p.querySelector('[data-push-help]')){const d=document.createElement('div');d.dataset.pushHelp='1';d.className='trend-push-help';d.innerHTML='<b>🔔 Kostenfreie Hintergrund-Pushs</b><span>Für echte Benachrichtigungen bei geschlossener App: die kostenlose ntfy-App installieren. „🔔 Push“ öffnet das persönliche Thema und abonniert es direkt.</span><a href="https://ntfy.sh/" target="_blank" rel="noopener">ntfy Web öffnen</a>';p.querySelector('.watchlist-body')?.prepend(d)}
 p.querySelectorAll('.watch-row').forEach(row=>{if(row.querySelector('[data-push]'))return;const w=row.querySelector('.watch-wkn')?.textContent?.replace(/^WKN:\s*/i,'').trim();if(!w)return;const b=document.createElement('button');b.type='button';b.dataset.push='1';b.textContent='🔔 Push';b.title='Kostenfreies ntfy-Push-Thema abonnieren';b.onclick=()=>{location.href=topicUrl(w)};row.appendChild(b)})}
