...
# WKN ist für die Kursberechnung optional, der Ticker ist maßgeblich.
CANDIDATES=[
# USA
...
# Nie aus einem unvollständigen Abruf eine gute Top-10-Liste verkleinern.
if len(items)<6:
 try:
...
out={'generatedAt':datetime.datetime.now(datetime.timezone.utc).isoformat(),'criteria':'Weltweites Kandidatenuniversum; identisches AKTScore-Scoring 0-100; nur Titel mit Score >=90/100; maximal 10 Titel. Bei unvollständigem Abruf bleibt der letzte vollständige >=90-Snapshot erhalten.','items':items}
with open('top10.json','w',encoding='utf-8') as f:json.dump(out,f,ensure_ascii=False,separators=(',',':'));f.write('\n')
print('Candidates:',len(CANDIDATES),'Top10:',[(x['symbol'],x['score']) for x in items])
