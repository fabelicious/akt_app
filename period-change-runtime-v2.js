...
function init(){const s=document.createElement('style');s.textContent='.period-change-card{padding:7px 8px!important;min-height:0}.period-change-card .label{font-size:8px}.period-change-card .value{font-size:14px;margin-top:1px}.period-change-card .sub{font-size:8px}.period-change-card .positive{color:#15803d}.period-change-card .negative{color:#b91c1c}.period-change-card .neutral{color:#64748b}';document.head.appendChild(s);const root=document.getElementById('individuals');if(root)new MutationObserver(updateAll).observe(root,{childList:true,subtree:true});document.addEventListener('click',e=>{if(e.target.closest('.tab'))setTimeout(updateAll,650)});document.addEventListener('change',e=>{if(e.target.closest('.venue-select'))setTimeout(updateAll,650)});[300,700,1200,2000,3500].forEach(t=>setTimeout(updateAll,t))}
